...
**T9. A gate never builds, tags or runs the image tag production runs from — its own tag is separate and disposable — and a deploy builds the production tag itself, from the merged tree.** A gate that reuses the production tag hands every branch a write to what production is recreated from: Memento's live container came up carrying `gd` and `exif` that only an unmerged branch builds, because a gate run on that branch had overwritten `:latest`. — *checked by:* `scripts/gate-image-tags.sh <project-root>` in the gate — it splits the compose files into gate ones (`*e2e*`, `*ci*`) and production ones, resolves `${VAR:-default}`, and fails naming the file and line when one built tag is on both sides; and after a deploy, `scripts/verify.sh` comparing `docker inspect -f '{{.Image}}'` on each running container against the image id that deploy built — the tag is never the evidence, the id is.

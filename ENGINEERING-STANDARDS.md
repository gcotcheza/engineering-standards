...
**W4. A PR's title is one past-tense action with no file names, and its body uses these four headings, literally, in ≤150 words of plain language.** *Added caching for card images* — never *The phone never caches card images*: a merge list read months later is a list of what was done, and a title that states the problem makes the reader open the PR to find out whether it was solved. The problem belongs under `## Why`. The body is for the person deciding to merge, not for the developer who wrote it. — *checked by:* the reviewer, before the PR goes ready, who corrects a problem-statement title with `gh pr edit <n> --title`:
